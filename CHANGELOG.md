...
### Fixed
- **Authentication settings: the API Key field now explains why it can appear blank.** When authentication is disabled, API-key management is intentionally restricted to local/private-network callers, so a client reaching Listenarr from outside that boundary (e.g. via a VPN such as Tailscale, where the forwarded client IP is outside RFC1918, or some reverse-proxy setups) receives a 403 that `SettingsView` swallowed into an empty field with a greyed copy button and a no-op regenerate button — with no explanation. The API Key field's help text now states that the key is only revealed to signed-in administrators or to clients on a local/private network, and tells the user to enable authentication and sign in (or open Listenarr from a local/trusted network) to view, copy, or regenerate it. The security gate itself is unchanged. ([#655](https://github.com/Listenarrs/Listenarr/issues/655))
